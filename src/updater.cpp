...
Updater::Updater(QObject *parent)
    : QObject(parent), m_state(Null), m_updateDetails(""), m_hasUpdate(false) {
  QObject::connect(this, &Updater::restarting, QCoreApplication::instance(),
                   &QCoreApplication::exit, Qt::QueuedConnection);
  QObject::connect(&m_check, &QProcess::finished, this,
                   &Updater::checkfinished);
  QObject::connect(&m_update, &QProcess::finished, this,
...
void Updater::restart() {
  if (m_state == ReadyToRestart) {
    ...
    qint64 pid;
    emit restarting(0);
    auto started = QProcess::startDetached(
        QCoreApplication::applicationFilePath(), args, QString(), &pid);
    qDebug() << "started:" << started << " pid:" << pid;
  }
}
void Updater::finishUpdate(int exitCode, QProcess::ExitStatus exitStatus) {
  qDebug() << "finishUpdate exitCode=" << exitCode
...
void Updater::checkfinished(int exitCode, QProcess::ExitStatus exitStatus) {
  qDebug() << "checkfinished exitCode=" << exitCode
           << ", exitStatus=" << exitStatus;
  ...
  if (exitCode == 0) {
    ...
    if (xmlindex != -1) {
      ...
      if (!updates.isNull()) {
        for (auto n = updates.firstChildElement("update"); !n.isNull();
             n = n.nextSiblingElement("update")) {
...
void Updater::hasIFW(QString toolname, bool absolute) {
  ...
  if (absolute) {
    ...
    toolpath =
        QDir(QCoreApplication::applicationDirPath()).absoluteFilePath(toolname);
  }
